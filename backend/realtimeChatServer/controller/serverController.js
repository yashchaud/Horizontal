...
async function saveServer(res, serverName, req, originalname, fileLocation) {
  const newServer = new Server({
    ...
    description: "Welcome to the " + serverName + " server",
  ...
  newServer.defaultChannel = newChannel._id;
...
const serverController = {
  ...
  CreateServerInvite: async (req, res, next) => {
    try {
      ...
      const inviteLink = randomstring.generate(7);
      server.inviteLink = inviteLink;
      console.log(server);
      await server.save();
      res.json({ message: "Invite link generated successfully.", inviteLink });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
  ...
  joinServer: async (req, res, next) => {
    try {
      const { serverId, inviteLink } = req.params;
