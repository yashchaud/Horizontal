...
import { setScrollToMessageId } from "@/Redux/sessionSlice";
const Chatsection = () => {
  const { channelId, threadId } = useParams();
  const { Thread, scrollToMessageId } = useSelector(
  ...
  useEffect(() => {
    ...
    if (scrollToMessageId && messageRefs.current[scrollToMessageId]) {
      const messageElement = messageRefs.current[scrollToMessageId].current;
      if (messageElement) {
        messageElement.scrollIntoView({ behavior: "smooth", block: "center" });
        // Optionally clear the scrollToMessageId state
        dispatch(setScrollToMessageId(null));
      }
    }
  }, [scrollToMessageId, dispatch]);
